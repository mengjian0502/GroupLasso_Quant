...
def make_layers_quant(cfg, batch_norm=False, wbit=4, abit=4, alpha_init=10, mode='mean', k=2, col_size=16, ch_group=16, ADCprecision=5, cellBit=2):
    layers = list()
    in_channels = 3
    for v in cfg:
        if v == 'M':
            ...
            if in_channels == 3:
                conv2d = int_conv2d(in_channels, v, kernel_size=3, padding=1, bias=False, nbit=wbit, mode=mode, k=k, ch_group=ch_group, push=False)
            else:
                conv2d = Qconv2d_adc(in_channels, v, kernel_size=3, padding=1, bias=False, 
...
class VGG_quant(nn.Module):
    def __init__(self, num_classes=10, depth=16, batch_norm=False, wbit=4, abit=4, alpha_init=10, mode='mean', k=2, col_size=16, ch_group=16, ADCprecision=5, cellBit=2):
        super(VGG_quant, self).__init__()
        self.features = make_layers_quant(cfg[depth], batch_norm, wbit=wbit, abit=abit, alpha_init=alpha_init, mode=mode, k=k, ch_group=ch_group, ADCprecision=ADCprecision, cellBit=cellBit)
        if depth == 7:
            self.classifier = nn.Sequential(
                int_linear(8192, 1024, nbit=wbit, mode=mode, k=k, ch_group=ch_group, push=False),
                # nn.Linear(8192, 1024),
                ClippedReLU(num_bits=abit, alpha=alpha_init, inplace=True),
                int_linear(1024, num_classes, nbit=wbit, mode=mode, k=k, ch_group=ch_group, push=False),
                # nn.Linear(1024, num_classes),
            )
        else:
            self.classifier = nn.Sequential(
